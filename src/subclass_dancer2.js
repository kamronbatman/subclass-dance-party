var makeGlyphDancer = function(top, left, timeBetweenSteps){
  makeDancer.apply(this, arguments);

  this.fa = '';
}

makeGlyphDancer.prototype = Object.create(makeBlinkyDancer.prototype);
makeGlyphDancer.prototype.constructor = makeGlyphDancer;

makeGlyphDancer.prototype.step = function(){
  if (this.fa != null) { this.$node.removeClass(this.fa); }

  var glyphs = makeGlyphDancer.prototype.glyphs;
  var random = Math.floor(Math.random() * glyphs.length);
  this.fa = glyphs[random];

  this.$node.addClass(this.fa);
  this.$node.css({ 'color': '#' + (Math.floor(Math.random() * 65536)).toString(16)});

  var randSize = Math.random() * 5;
  this.$node.css({
      '-webkit-transform': 'scale(' + randSize + ',' + randSize + ')',
      'transform': 'scale(' + randSize + ',' + randSize + ')'
  });

  makeDancer.prototype.step.call(this);
}

makeGlyphDancer.prototype.createNode = function(){
  makeBlinkyDancer.prototype.createNode.call(this);
  this.$node.addClass("fa");
  this.$node.css({ border: '0px' });
}

makeGlyphDancer.prototype.glyphs = Object.keys({
    "fa-adjust": "Adjust",
    "fa-adn": "Adn",
